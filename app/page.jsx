...
import {
  ...
  Coins, RotateCcw, Sparkles, RefreshCw, CalendarDays, CalendarClock,
} from "lucide-react";
...
/* ---------------- dividend-calendar / events helpers -------------- */
const WD = ["日", "月", "火", "水", "木", "金", "土"];

/* split each holding's annual dividend evenly across its payout months */
function monthlyDividends(holdings) {
  const arr = Array(12).fill(0);
  let unassigned = 0;
  holdings.forEach((h) => {
    const annual = (h.shares || 0) * (h.dividendPerShare || 0);
    if (annual <= 0) return;
    const months = Array.isArray(h.dividendMonths)
      ? h.dividendMonths.filter((m) => m >= 1 && m <= 12)
      : [];
    if (!months.length) { unassigned += annual; return; }
    const per = annual / months.length;
    months.forEach((m) => { arr[m - 1] += per; });
  });
  return { arr, unassigned, total: arr.reduce((a, b) => a + b, 0) };
}
...
/* collect future earnings + record dates, soonest first */
function upcomingEvents(holdings) {
  const t0 = todayStart();
  const out = [];
  holdings.forEach((h) => {
    const e = parseDate(h.earningsDate);
    if (e) out.push({ type: "earnings", date: e, name: h.name, ticker: h.ticker });
    const r = parseDate(h.recordDate);
    if (r) out.push({ type: "record", date: r, name: h.name, ticker: h.ticker });
  });
  return out
    .filter((x) => daysBetween(x.date, t0) >= 0)
  ...
  return (
    <main className="min-h-screen w-full">
      <div className="mx-auto w-full max-w-2xl px-4 pb-12 pt-6">
        ...
        {holdings.length > 0 && <UpcomingEvents holdings={holdings} />}

        {holdings.length > 0 && <DividendCalendar holdings={holdings} />}
...
function DividendCalendar({ holdings }) {
  const { arr, unassigned, total } = useMemo(() => monthlyDividends(holdings), [holdings]);
  if (total <= 0 && unassigned <= 0) return null;
  const max = Math.max(...arr, 1);
  const thisMonth = new Date().getMonth(); // 0-11

  return (
    <section className="mt-4 rounded-2xl p-4" style={{ background: "#fff", border: `1px solid ${C.border}` }}>
      <div className="mb-3 flex items-center gap-1.5 text-xs font-semibold" style={{ color: C.muted }}>
        <CalendarDays size={14} color={C.gold} /> 配当カレンダー（月別の受取見込み）
      </div>

      <div className="space-y-1.5">
        {arr.map((v, i) => {
          const cur = i === thisMonth;
          return (
            <div key={i} className="flex items-center gap-2">
              <span className="num t11 w-8 shrink-0 text-right" style={{ color: cur ? C.goldDark : C.muted, fontWeight: cur ? 700 : 400 }}>
                {i + 1}月
              </span>
              <div className="relative h-5 flex-1 overflow-hidden rounded-md" style={{ background: "#F1F4F9" }}>
                {v > 0 && (
                  <div className="seg h-full rounded-md" style={{ width: Math.max((v / max) * 100, 2) + "%", background: cur ? C.gold : "#D9C68A" }} />
                )}
              </div>
              <span className="num t11 w-20 shrink-0 text-right" style={{ color: v > 0 ? C.ink : "#C2C9D6" }}>
                {v > 0 ? yen(v) : "—"}
              </span>
            </div>
          );
      ...
      <div className="mt-3 flex items-center justify-between border-t pt-2.5" style={{ borderColor: C.border }}>
        <span className="t11" style={{ color: C.muted }}>年間合計</span>
        <span className="num text-sm font-bold" style={{ color: C.goldDark }}>{yen(total)}</span>
      </div>

      {unassigned > 0 && (
        <p className="mt-2 t11 leading-relaxed" style={{ color: C.muted }}>
          支払月が未設定の配当が {yen(unassigned)} あります。銘柄の編集で「配当の支払月」を選ぶとこの表に反映されます。
        </p>
      )}
    </section>
  );
...
/* ----------------------- Upcoming events -------------------------- */
function UpcomingEvents({ holdings }) {
  const events = useMemo(() => upcomingEvents(holdings), [holdings]);
  const hasDates = holdings.some((h) => h.earningsDate || h.recordDate);

  if (!hasDates) {
    return (
      <section className="mt-4 rounded-2xl p-4" style={{ background: "#fff", border: `1px solid ${C.border}` }}>
        <div className="mb-1 flex items-center gap-1.5 text-xs font-semibold" style={{ color: C.muted }}>
          <CalendarClock size={14} color={C.gold} /> この先の予定
        </div>
        <p className="t11 leading-relaxed" style={{ color: C.muted }}>
          銘柄の編集で「決算予定日」や「権利確定日」を入れると、近い順にここへ並びます。
        </p>
      </section>
    );
  }
  ...
  return (
    <section className="mt-4 rounded-2xl p-4" style={{ background: "#fff", border: `1px solid ${C.border}` }}>
      ...
      {events.length === 0 ? (
        <p className="t11" style={{ color: C.muted }}>登録した日付はすべて過ぎています。編集から次回の日付に更新できます。</p>
      ) : (
        <ul className="space-y-2">
          {shown.map((e, i) => {
            const dleft = daysBetween(e.date, t0);
            const soon = dleft <= 7;
            ...
            return (
              <li
                ...
                style={{ background: soon ? "rgba(200,162,75,.10)" : "#FAFBFD", border: `1px solid ${soon ? "rgba(200,162,75,.35)" : C.border}` }}
              >
                <div className="flex w-11 shrink-0 flex-col items-center">
                  <span className="num text-sm font-bold" style={{ color: soon ? C.goldDark : C.ink }}>{fmtMD(e.date)}</span>
                  <span className="t10" style={{ color: C.muted }}>({WD[e.date.getDay()]})</span>
                </div>
                <div className="min-w-0 flex-1">
                  <div className="truncate t15 font-semibold" style={{ color: C.ink }}>{e.name}</div>
                  <div className="mt-0.5 flex items-center gap-1.5">
                    <span className="t10 rounded px-1.5 py-0.5 font-semibold" style={{ color: isEarn ? "#2A4E80" : C.goldDark, background: isEarn ? "#EAF0F9" : "rgba(200,162,75,.15)" }}>
                      {isEarn ? "決算" : "権利確定"}
                    </span>
                    {e.ticker && <span className="num t10" style={{ color: C.muted }}>{e.ticker}</span>}
                  </div>
                </div>
                <span className="num t11 shrink-0 text-right" style={{ color: soon ? C.goldDark : C.muted, fontWeight: soon ? 700 : 400 }}>
                  {dleft === 0 ? "今日" : `あと${dleft}日`}
                </span>
              </li>
            );
          })}
      ...
      {events.length > shown.length && (
        <p className="mt-2 text-center t11" style={{ color: C.muted }}>ほか {events.length - shown.length} 件</p>
      )}
    </section>
  );
}
...
function Shell({ children, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center" style={{ background: "rgba(10,16,30,.55)" }} onClick={onClose}>
      <div
        className="row-in max-h-[92vh] w-full max-w-md overflow-y-auto rounded-t-3xl p-5 sm:rounded-3xl"
...
function FormModal({ initial, onClose, onSave }) {
  const [f, setF] = useState({
    ...
    dividendMonths: Array.isArray(initial?.dividendMonths) ? initial.dividendMonths : [],
    earningsDate: initial?.earningsDate ?? "",
  ...
  const toggleMonth = (m) =>
    setF((p) => ({
      ...p,
      dividendMonths: p.dividendMonths.includes(m)
        ? p.dividendMonths.filter((x) => x !== m)
        : [...p.dividendMonths, m].sort((a, b) => a - b),
    }));
  ...
  const submit = () => {
    ...
    onSave({
      ...
      dividendMonths: f.dividendMonths,
  ...
  return (
    <Shell onClose={onClose}>
      ...
      <div className="mt-1 mb-1 flex items-center gap-1.5 t11 font-semibold" style={{ color: C.gold }}>
        <CalendarDays size={13} /> 配当・予定（任意）
      </div>

      <div className="mb-3">
        <span className="mb-1.5 block text-xs font-medium" style={{ color: C.muted }}>
          配当の支払月<span className="ml-1" style={{ color: "#A6AEBC" }}>（年間配当をこの月数で等分）</span>
        </span>
        <div className="grid grid-cols-6 gap-1.5">
          {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => {
            const on = f.dividendMonths.includes(m);
            return (
              <button
                key={m}
                type="button"
                onClick={() => toggleMonth(m)}
                className="ix num rounded-lg py-2 text-xs font-semibold"
                style={{
                  color: on ? C.panel : C.muted,
                  background: on ? C.gold : "#F3F6FB",
      ...
      <div className="grid grid-cols-2 gap-3">
        <DateInput label="決算予定日" value={f.earningsDate} onChange={set("earningsDate")} />
        <DateInput label="権利確定日" value={f.recordDate} onChange={set("recordDate")} />
      </div>
...
function DateInput({ label, value, onChange }) {
  return (
    <label className="mb-3 block">
      <span className="mb-1 block text-xs font-medium" style={{ color: C.muted }}>{label}</span>
      <span className="flex items-center rounded-xl border bg-white px-3" style={{ borderColor: C.border }}>
        <input
          type="date"
          value={value}
          onChange={onChange}
          className="fld num t15 w-full bg-transparent py-2.5"
