...
const SAMPLE = [
  { name: "トヨタ自動車", ticker: "7203.T", shares: 100, avgPrice: 2800, currentPrice: 3200, dividendPerShare: 90, currency: "JPY" },
  { name: "三菱UFJ FG", ticker: "8306.T", shares: 200, avgPrice: 1200, currentPrice: 1850, dividendPerShare: 60, currency: "JPY" },
  { name: "NTT", ticker: "9432.T", shares: 300, avgPrice: 160, currentPrice: 155, dividendPerShare: 5.2, currency: "JPY" },
];
...
/* currency-aware: USD shows "$" with 2 decimals, JPY shows "¥" rounded */
const sym = (cur) => (cur === "USD" ? "$" : "¥");
const money = (n, cur) =>
  cur === "USD"
    ? "$" + (n || 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })
    : "¥" + Math.round(n || 0).toLocaleString("ja-JP");
const signedMoney = (n, cur) => (n >= 0 ? "+" : "−") + money(Math.abs(n || 0), cur);
const price2 = (n, cur) =>
  cur === "USD"
    ? (n || 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })
    : Number.isInteger(n)
    ? n.toLocaleString("ja-JP")
    : (n || 0).toLocaleString("ja-JP", { maximumFractionDigits: 2 });
/* convert a native-currency amount into JPY for the combined totals */
const toJpy = (n, cur, rate) => (cur === "USD" ? (n || 0) * (rate || 0) : n || 0);
const clock = (ts) =>
...
export default function Page() {
  ...
  const [usdJpy, setUsdJpy] = useState(150); // refreshed from Yahoo on 更新
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed && Array.isArray(parsed.holdings)) {
          ...
          if (typeof parsed.usdJpy === "number" && parsed.usdJpy > 0) setUsdJpy(parsed.usdJpy);
  ...
  useEffect(() => {
    if (!loaded) return;
    try {
      localStorage.setItem(STORE_KEY, JSON.stringify({ holdings, lastUpdated, usdJpy }));
    } catch (e) {
      /* storage full / unavailable — keep working in memory */
    }
  }, [holdings, lastUpdated, usdJpy, loaded]);

  const totals = useMemo(() => {
    let cost = 0, value = 0, annualDiv = 0, hasUsd = false;
    holdings.forEach((h) => {
      const d = compute(h);
      const rate = h.currency === "USD" ? usdJpy : 1;
      if (h.currency === "USD") hasUsd = true;
      cost += d.cost * rate;
      value += d.value * rate;
      annualDiv += d.annualDiv * rate;
    });
    const pl = value - cost;
    return {
      cost, value, pl,
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (typeof json.usdJpy === "number" && json.usdJpy > 0) setUsdJpy(json.usdJpy);
      const map = {};
      (json.quotes || []).forEach((q) => { if (q.ok) map[q.symbol] = q; });
      const okCount = Object.keys(map).length;

      setHoldings((prev) =>
        prev.map((h) => {
          const q = h.ticker && map[h.ticker.trim()];
          if (!q) return h;
          const next = { ...h, currentPrice: q.price };
          if (typeof q.annualDividend === "number") {
            next.dividendPerShare = q.annualDividend;
          }
          if (q.currency === "USD" || q.currency === "JPY") {
            next.currency = q.currency;
          }
          return next;
        })
      );
  ...
  return (
    <main className="min-h-screen w-full">
      <div className="mx-auto w-full max-w-2xl px-4 pb-12 pt-6">
        ...
        {holdings.length > 0 && <Allocation holdings={holdings} totalValue={totals.value} usdJpy={usdJpy} />}
          ...
          <ul className="mt-5 space-y-3">
            {holdings.map((h, i) => (
              <HoldingCard
                key={h.id}
                h={h}
                weight={totals.value > 0 ? (toJpy(compute(h).value, h.currency, usdJpy) / totals.value) * 100 : 0}
...
/* -------------------------- Summary board ------------------------- */
function SummaryBoard({ t, count, onRefresh, refreshing, lastUpdated, canRefresh, usdJpy }) {
  const up = t.pl >= 0;
  return (
    <section className="overflow-hidden rounded-3xl" style={{ background: C.panel, boxShadow: "0 16px 40px rgba(14,31,58,.28)" }}>
      <div className="px-6 pt-6 pb-5">
        ...
        {t.hasUsd && (
          <div className="num mt-2 t10" style={{ color: C.onNavyMuted }}>
            米国株は 1 $ = ¥{price(usdJpy)} で円換算（合計は円表示・更新で最新化）
          </div>
        )}
      </div>
...
/* ---------------------- Allocation ribbon ------------------------- */
function Allocation({ holdings, totalValue, usdJpy }) {
  const items = holdings
    .map((h, i) => ({ name: h.name, w: totalValue > 0 ? (toJpy(compute(h).value, h.currency, usdJpy) / totalValue) * 100 : 0, c: SEG[i % SEG.length] }))
    .filter((x) => x.w > 0)
    .sort((a, b) => b.w - a.w);
  ...
  return (
    <li className="row-in overflow-hidden rounded-2xl" style={{ background: "#fff", border: `1px solid ${C.border}`, boxShadow: "0 2px 8px rgba(19,32,59,.04)" }}>
      <div className="flex items-start gap-3 px-4 pt-3.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="num mt-0.5 text-xs" style={{ color: C.muted }}>
            {price(h.shares)}株 · 取得 {sym(h.currency)}{price2(h.avgPrice, h.currency)} → 現在 {sym(h.currency)}{price2(h.currentPrice, h.currency)}
          </div>
        </div>
        <div className="text-right">
          <div className="num t15 font-bold" style={{ color: plColor }}>{signedMoney(d.pl, h.currency)}</div>
      ...
      <div className="mt-3 grid grid-cols-3 border-t" style={{ borderColor: C.border }}>
        <Cell label="評価額" value={money(d.value, h.currency)} />
        <Cell label="年間配当" value={money(d.annualDiv, h.currency)} divider />
...
function FormModal({ initial, onClose, onSave }) {
  const [f, setF] = useState({
    ...
    currency: initial?.currency === "USD" ? "USD" : "JPY",
  ...
  const submit = () => {
    ...
    onSave({
      ...
      currency: f.currency === "USD" ? "USD" : "JPY",
      shares: num(f.shares),
  ...
  return (
    <Shell onClose={onClose}>
      ...
      <div className="mb-3">
        <span className="mb-1 block text-xs font-medium" style={{ color: C.muted }}>通貨</span>
        <div className="flex gap-2">
          {[["JPY", "円 ¥"], ["USD", "ドル $"]].map(([v, lbl]) => (
            <button
              key={v}
              type="button"
              onClick={() => setF((p) => ({ ...p, currency: v }))}
              className="ix flex-1 rounded-xl py-2.5 text-sm font-semibold"
              style={
                f.currency === v
                  ? { background: C.panel, color: "#fff" }
        ...
        <p className="mt-1 t10" style={{ color: "#A6AEBC" }}>
          米国株は「ドル」を選び、取得単価・現在値もドルで入力してください
        </p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <Input label="株数" unit="株" value={f.shares} onChange={set("shares")} numeric />
        <Input label="取得単価" unit={unit} value={f.avgPrice} onChange={set("avgPrice")} numeric />
        <Input label="現在値" unit={unit} value={f.currentPrice} onChange={set("currentPrice")} numeric hint="更新で上書き" />
        <Input label="1株あたり年間配当" unit={unit} value={f.dividendPerShare} onChange={set("dividendPerShare")} numeric hint="なければ 0" />
      </div>
